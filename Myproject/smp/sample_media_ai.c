...
HI_S32 regions_Create( RGN_HANDLE RgnHandle, int width, int height, int position_x, int position_y )
{
    ...
    ****************************************/
    ...
    stRgnAttr.unAttr.stOverlayEx.stSize.u32Width = width;
    stRgnAttr.unAttr.stOverlayEx.stSize.u32Height = height;
    ...
    ****************************************/    
    ...
    stChnAttr.unChnAttr.stOverlayExChn.u32BgAlpha = 255;  	        //OSD Alpha
...
HI_S32 SAMPLE_RGN_LoadBmp(const char* filename, BITMAP_S* pstBitmap)
{
    ...
    pstBitmap->u32Width = *(int*)&header[18];
    pstBitmap->u32Height = *(int*)&header[22];
    pstBitmap->pData = NULL;

    // Calculate stride
    int bpp = *(int*)&header[28] / 8;
    int stride = ((pstBitmap->u32Width * bpp + 3) >> 2) << 2;

    // Allocate memory for image data
    pstBitmap->pData = malloc(pstBitmap->u32Width * pstBitmap->u32Height * 4);
    ...
    for (int y = 0; y < pstBitmap->u32Height; y++) {
       for (int x = 0; x < pstBitmap->u32Width; x++) {
        uint8_t pixel[3];
        fread(pixel, 1, 3, file);

        // Convert BGR to ARGB 8888
        int index = (pstBitmap->u32Height - y - 1) * pstBitmap->u32Width + x;  // Calculate reversed index
        ((HI_U8*)pstBitmap->pData)[index * 4 + 3] = 255;   // Alpha
        ((HI_U8*)pstBitmap->pData)[index * 4 + 2] = pixel[2];  // Red
        ((HI_U8*)pstBitmap->pData)[index * 4 + 1] = pixel[1];  // Green
        ((HI_U8*)pstBitmap->pData)[index * 4 + 0] = pixel[0];  // Blue
        }
    }
...
HI_S32 update_regions(const char *filename, RGN_HANDLE RgnHandle)
{
    HI_S32 s32Ret = HI_FAILURE;
    BITMAP_S stBitmap;
    if( HI_SUCCESS != SAMPLE_RGN_LoadBmp(filename, &stBitmap))
    {
...
HI_S32 SampleCommVoStartChnMipi(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    ...
    for (i = 0; i < g_sampleVoModeMux.u32WndNum; i++) {
         ...
         */
        s32Ret = HI_MPI_VO_SetChnRotation(VoLayer, i, 1);
...
static HI_VOID CnnAiProcess(VIDEO_FRAME_INFO_S frm)
{
    ...
    if (GetCfgBool("trash_classify_switch:support_trash_classify", true)) {
        ...
        ret = CnnCal(g_workPlug.model, &frm, resFrm);
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, TRASH_RELEASE,
...
static HI_VOID* GetVpssChnFrameCnn(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    HI_S32 s32MilliSec = 100000;
...
static HI_S32 CnnAiThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    ...
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameCnn, NULL);
...
static HI_S32 PauseDoUnloadCnnModel(HI_VOID)
{
    ...
    if (GetCfgBool("trash_classify_switch:support_trash_classify", true)) {
         ...
         */
        s32Ret = CnnUnloadModel(g_workPlug.model);
 ...
 * VI->VPSS->VO->MIPI
 */
HI_S32 SAMPLE_MEDIA_CLASSIFY(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
